...
const leftCol = computed(() => displayItems.value.filter((_: Item, i: number) => i % 3 === 0))
const centerCol = computed(() => displayItems.value.filter((_: Item, i: number) => i % 3 === 1))
const rightCol = computed(() => displayItems.value.filter((_: Item, i: number) => i % 3 === 2))
...
.header-location { font-size: 13px; color: rgba(255,255,255,0.95); background: rgba(255,255,255,0.2); padding: 4px 12px; border-radius: 14px; }
.search-input {
  display: flex; align-items: center; background: #fff; border-radius: 22px;
  padding: 10px 16px; gap: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);
  input { flex: 1; font-size: 15px; color: #1d1d1f; }
  .search-icon { font-size: 15px; }
  .search-clear { width: 20px; height: 20px; border-radius: 50%; background: #c7c7cc; color: #fff; font-size: 11px; display: flex; align-items: center; justify-content: center; }
}

.category-bar { background: #fff; padding: 12px 8px 10px; white-space: nowrap; border-bottom: 1px solid #f0f0f0; }
.cat-item {
  display: inline-flex; flex-direction: column; align-items: center;
  padding: 6px 14px; margin: 0 2px; border-radius: 12px; gap: 2px; transition: all 0.2s; cursor: pointer;
  &.active { background: #FFF0E8; .cat-label { color: #FF6B35; font-weight: 600; } }
  &:hover { background: #fafafa; }
}
.cat-icon { font-size: 22px; }
.cat-label { font-size: 11px; color: #86868b; }

.feed { height: calc(100vh - 160px); }
.waterfall { display: flex; padding: 8px; gap: 8px; }
...
.card-badge {
  ...
  border-radius: 6px; font-size: 11px; font-weight: 600; backdrop-filter: blur(4px);
  &.new { background: rgba(255,107,53,0.9); color: #fff; }
  &.like-new { background: rgba(82,196,26,0.9); color: #fff; }
}
.card-body { padding: 10px 12px 12px; }
.card-title { font-size: 14px; color: #1d1d1f; line-height: 1.45; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; font-weight: 500; }
.card-price-row { display: flex; align-items: baseline; gap: 6px; margin-top: 8px; }
.card-price { font-size: 18px; font-weight: 800; color: #FF6B35; letter-spacing: -0.5px; }
.card-condition { font-size: 11px; color: #86868b; background: #f5f5f7; padding: 2px 6px; border-radius: 4px; }
.card-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 10px; padding-top: 8px; border-top: 1px solid #f5f5f7; }
.card-seller { display: flex; align-items: center; gap: 5px; }
.seller-avatar { width: 20px; height: 20px; border-radius: 50%; background: #f0f0f0; }
.seller-name { font-size: 11px; color: #86868b; max-width: 72px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.card-location { font-size: 10px; color: #aeaeb2; }

.loading-tip { display: flex; align-items: center; justify-content: center; padding: 24px; gap: 8px; color: #aeaeb2; font-size: 13px; }
.loading-dots { display: flex; gap: 2px;
  text { animation: blink 1.4s infinite both; font-size: 20px; color: #FF6B35;
    &:nth-child(2) { animation-delay: 0.2s; } &:nth-child(3) { animation-delay: 0.4s; }
  }
}
@keyframes blink { 0%, 80%, 100% { opacity: 0.2; } 40% { opacity: 1; } }
.divider-line { width: 32px; height: 1px; background: #d1d1d6; }
.empty-state { display: flex; flex-direction: column; align-items: center; padding-top: 120px; gap: 8px; }
.empty-icon { font-size: 56px; }
.empty-title { font-size: 17px; color: #1d1d1f; font-weight: 600; }
.empty-sub { font-size: 14px; color: #86868b; }
.empty-btn { margin-top: 16px; padding: 12px 40px; background: #FF6B35; color: #fff; border-radius: 22px; font-size: 15px; font-weight: 600; cursor: pointer; }
...
@media (min-width: 768px) {
  ...
  .feed { height: calc(100vh - 140px); }
  .waterfall { padding: 16px 24px; gap: 16px; }
  .waterfall-col { gap: 16px; }

  .card-body { padding: 14px 16px 16px; }
  .card-title { font-size: 15px; }
  .card-price { font-size: 20px; }
  .card-condition { font-size: 12px; }
  .card-footer { margin-top: 12px; padding-top: 10px; }
  .seller-avatar { width: 24px; height: 24px; }
  .seller-name { font-size: 13px; max-width: 100px; }
  .card-location { font-size: 12px; }
}

/* ===== WIDE DESKTOP (>= 1024px): 3 columns ===== */
@media (min-width: 1024px) {
  .waterfall { flex-wrap: wrap; }
  .waterfall-col { flex: none; width: calc(33.333% - 11px); }
}
</style>
