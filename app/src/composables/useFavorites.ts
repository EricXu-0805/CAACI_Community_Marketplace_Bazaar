...
export function useFavorites() {
  ...
  async function fetchMyFavoriteItems(userId: string) {
    ...
    const { data, error } = await supabase
      .from('favorites')
      .select('item_id, item:items(id, user_id, title, title_i18n, price, category, condition, status, listing_details, listing_type, location, images, image_dimensions, view_count, favorite_count, negotiable, created_at, profile:profiles(id, nickname, avatar_url, location))')
      .eq('user_id', userId)
