export const GUIDELINES_VERSION = '2026-04-20'

export const GUIDELINES_EN = `Community Guidelines

Version: ${GUIDELINES_VERSION}

Illini Market is a marketplace and a community. The Terms of Service set the legal boundary; these Guidelines set the cultural one. Stay inside both.

1. Be a decent human.
Treat other users the way you would treat a classmate standing in line at the Union. Disagreement is fine; contempt, hostility, targeted harassment, and hate speech are not. This includes dog-piling, coordinated mockery, and context-free screenshots of someone's post shared to humiliate them.

2. Post what you actually have.
Listings must describe the real item, in its real condition, available at a real price. No bait listings, no photos you pulled off Amazon for an item you don't own, no "click here for details on my WeChat". Add clear original photos whenever possible.

3. Keep conversations on Illini Market.
Use in-app chat for buyer–seller communication. Don't push buyers off-platform to WeChat, Discord, Instagram, or Telegram — this is how most scams on peer marketplaces happen. Listings and messages that push off-platform contact will be removed and your account may be rate-limited.

4. No contact info in public spaces.
Listings, plaza posts, comments, and item descriptions may not include phone numbers, WeChat IDs, QQ numbers, email addresses, or QR codes. Use in-app chat. This rule protects you more than it protects us.

5. Prohibited content at a glance.
   · weapons, ammunition, explosives;
   · controlled substances, prescription drugs, drug paraphernalia;
   · tobacco or alcohol sold to anyone underage;
   · stolen goods, counterfeits, items infringing trademarks;
   · currency exchange or money transmission — buying, selling, or exchanging USD, RMB, or any currency for another;
   · academic cheating services — writing essays, taking exams, selling exam answers, "homework helper" schemes;
   · adult content, escort services, sex work solicitation;
   · live animals and animal parts;
   · hazardous materials;
   · anything else illegal under US or Illinois law, or prohibited by UIUC policy.

6. Plaza-specific rules.
The Plaza is a community feed, not a classified ads board. Don't flood it with listings; use the Home tab for that. No coordinated self-promotion rings, no political campaigning, no religious proselytizing, no NSFW. If a post would be awkward on a classroom whiteboard, it doesn't belong here.

7. Messaging rules.
   · No unsolicited promotional messages.
   · No repeated messages after the other person asks you to stop.
   · No sending explicit content, ever.
   · Blocking a user is final from their side; don't create a new account to keep messaging them.

8. Reviews and reputation.
Do not fake reviews, trade reviews, or retaliate against users for leaving honest reviews of your listings. Submitting false reports to harm another user is itself a violation.

9. One person, one account.
Multiple accounts operated by the same person to inflate reach or evade moderation are not allowed. If you need a separate account for a legitimate reason (club, group project, etc.), email us first and we'll set it up properly.

10. No scraping, no bots, no API abuse.
Don't automate account creation, scrape listings, or pound our APIs. If you're building something cool and want data access, email us — we'll probably help.

11. Safe meetups.
When meeting another user in person:
   · Meet in public, ideally at a designated UIUC Safe Zone (library, Union, police station lobby).
   · Inspect the item before paying.
   · Never wire money or pay a deposit before the meetup.
   · Bring a friend for higher-value trades.
   · If something feels off, it is off — leave.

12. Enforcement.
Violations of these Guidelines are treated as violations of the Terms of Service and are enforced through the same ban ladder (ToS §9). Severe violations — threats, doxxing, illegal-goods listings, content involving minors — skip directly to a permanent ban.

13. Reporting.
If you see something that breaks these Guidelines, tap the "…" menu on any listing, post, comment, or user profile and choose "Report". Reports are reviewed by a human. False or malicious reports are themselves a violation.

14. Feedback.
These Guidelines evolve. If a rule feels off, write us at help@illinimarket.com and tell us why — we read every message.
`
