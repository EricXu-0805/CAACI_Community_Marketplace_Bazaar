...
-- trg_enforce_actor to record their events. Adds admin_list_audit_log
-- RPC to feed the new "Audit log" tab in the admin dashboard.
--
-- Dependencies: 027 + 028 + 029 + 030 already applied.
-- Re-running is a safe no-op (OR REPLACE + IF NOT EXISTS everywhere).
-- =========================================================
...
BEGIN
  ...
  PERFORM public.record_audit(
    ...
    auth.uid(),
    report_id_in,
    jsonb_build_object('from', old_status, 'to', status_in)
  );
END;
$$;
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL AND actor_id <> auth.uid() THEN RETURN NEW; END IF;

  active_level := (SELECT p.suspension_level FROM public.profiles p WHERE p.id = actor_id);
  ends_at      := (SELECT p.suspended_until  FROM public.profiles p WHERE p.id = actor_id);
  ...
  IF active_level IS NOT NULL
    ...
    PERFORM public.record_audit(
      ...
      actor_id,
      NULL,
      jsonb_build_object('table', TG_TABLE_NAME, 'level', active_level, 'ends_at', ends_at)
    );
