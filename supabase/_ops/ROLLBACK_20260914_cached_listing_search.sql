-- Run the entire rollback in one transaction. Restores the exact pre-cache
-- RPC bodies; item data, moderation, write policies and other extensions stay.
...
DROP TRIGGER sync_listing_search_insert ON public.items;
DROP TRIGGER sync_listing_search_update ON public.items;
DROP FUNCTION search_private.sync_item_fields();
DROP TABLE search_private.item_fields;
DROP FUNCTION search_private.build_fields(text,jsonb,text,jsonb,jsonb);
DROP FUNCTION search_private.item_rank(search_private.cached_field[],search_private.term_input[],text[],real,boolean,boolean,boolean,integer,integer);
DROP FUNCTION search_private.token_ids(text);
DROP TYPE search_private.cached_field;
DROP TYPE search_private.term_input;
-- RESTRICT is intentional: unexpected dependencies must block removal.
DROP EXTENSION intarray;
DROP SCHEMA search_private;
NOTIFY pgrst, 'reload schema';
