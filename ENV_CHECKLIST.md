...
|---|---|---|
| Site URL | Auth → URL Configuration | `https://www.illinimarket.com` — the **`www` host**, because that is the origin the app actually runs on. The apex 308-redirects to `www`, so `window.location.origin` (which is what `signInWithOAuth` sends as `redirectTo`) is always the `www` form. Set 2026-08-14; it was the apex before. |
| Redirect URLs | Auth → URL Configuration | must include `https://www.illinimarket.com/**`. Added 2026-08-14 — the list previously held only the apex, so every OAuth `redirectTo` missed the allow-list and silently fell back to Site URL, discarding the app's intended return route. Keep the apex entry too. |
...
| Password policy | Auth → Policies | At least 8 characters + upper/lower/digit/symbol. Leaked-password (HIBP) protection is **ENABLED** in production — read directly off Auth → Attack Protection on 2026-08-14, and the Security Advisor no longer reports the finding. Earlier revisions of this row said it was OFF; that is stale. |
| Reset-password email | Auth → Email Templates → Reset Password | body uses `{{ .Token }}` (6-digit code, **not** the link) **and** Email OTP length = 6 (Auth → Providers → Email). The app's reset is a typed code (QA6 #138). |
| Confirm-signup email | Auth → Email Templates → Confirm sign up | body must carry the **6-digit code**, not a link. `pages/login/index.vue` renders a `UCodeInput` and calls `verifyOtp({ type: 'signup' })`, so a link-only template dead-ends every signup. Production is already on the code template (read off the dashboard 2026-08-14). Earlier revisions of this file and `RUNBOOK.md` said to leave this template on the link — that instruction was wrong, not merely stale. |
...
- [ ] Deployment gate is green: Vite refuses tier/project/origin drift; all 20 Supabase-backed Functions return 503 before upstream work on drift; the deployed project has 21 runtime Functions in total including the non-Supabase custom 404 boundary; `deployment-manifest.json` records a non-secret artifact identity. (These two counts are asserted by `api/_deployment-boundary.test.mjs` and `api/_edge-redirect-runtime-boundary.test.mjs`; the prose said 19/20 until 2026-08-14, which was one behind the tests.)
