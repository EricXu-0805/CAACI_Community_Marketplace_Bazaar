...
- [ ] **Take down the personal content before anyone is invited.** Two production rows carry photographs of an identifiable child and one of them also carries a residential address, and neither is behind a login: `robots.txt` is `Allow: /` and deliberately invites crawlers to the share endpoints, `/share/<id>` answers 200 with an `og:image` pointing straight at the photo, that page carries no `noindex`, and the file itself sits in a public storage bucket that needs no session. Verified on the live site 2026-08-29.
  - post `eb00eedf-67f8-4849-b4e3-d92d7ce1db33` — "young xgy", 4 images
  - item `4480d237-f5e8-4db4-b738-8dacef19ea3d` — 9 images, and the address in its location field
  - Two plaza posts are prompt-injection test strings, and they are the first thing on the Plaza tab.
  This is a different problem from the seeding item below. Seeding is about the feed looking dead; this is about what is already public, and it does not wait for the beta.
- [ ] Decide what to do about the four accounts that show the Illini badge with nothing behind it — `public.profiles` has 4 rows with `is_illini_verified = true` while `public.illini_verifications` is empty (2026-08-29). The badge cannot be forged: `anon` and `authenticated` hold only `SELECT` on that column and `guard_illini_verify_columns` is a second gate, so these are development residue rather than a hole. It still spends the trust signal the beta depends on.
