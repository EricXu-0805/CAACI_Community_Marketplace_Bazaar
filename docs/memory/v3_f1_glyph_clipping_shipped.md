---
name: F1 + F1b attempted → F1c shipped — Onboarding nickname input glyph clipping (2026-05-19)
description: F1 (line-height: 1.5 on .input) failed iPhone Safari real-device verification (~50% → ~10% glyph visible). F1b (height: 48px override) cleared the framework height cap but EXPOSED a new bug — uni-app's `.uni-input-placeholder` is an absolutely-positioned overlay element that sits at wrapper top and overlaps value text once the wrapper has visible height. F1c hides the placeholder via `:deep(.uni-input-placeholder) { display: none; }` (3 LOC). F1 + F1b commits preserved on branch as audit-failed → reality-check learning chain. Forward-add only per opencode_no_self_decided_history_rewrite.md. 3 new lesson files captured. New HARD rule: iOS Safari fixes need real-device gate before squash-merge.
type: project
originSessionId: opencode-f1c-fix-session
---
# F1 + F1b attempted → F1c shipped — Onboarding nickname input glyph clipping (2026-05-19)

## Status: F1 + F1b FAILED real-device → F1c SHIPPED via PR #21, squash `<TBD-sha>`
...
- New lesson on placeholder overlay: `docs/memory/lesson_uni_app_placeholder_overlay.md`
- Sprint tracker (v3.5 GC audit + F1 + F1b + F1c row): `docs/memory/sprint_v3_phase_status.md`
- Backlog (status now FIX SHIPPED via F1c): `docs/memory/backlog_onboarding_glyph_clipping.md`
- Workflow step 4 — memory rides with deliverable: `docs/memory/workflow_audit_first.md`
- No-history-rewrite rule (why F1 + F1b commits are preserved on the branch): `docs/memory/opencode_no_self_decided_history_rewrite.md`
