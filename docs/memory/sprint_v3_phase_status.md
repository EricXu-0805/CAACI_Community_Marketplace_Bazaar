...
| **v3.5 GC audit + F1c fix** | Onboarding step 1 nickname input glyph clipping | ✅ **AUDIT SHIPPED 2026-05-12 PR #17 + F1 attempted 2026-05-17 PR #20 (closed, real-device fail) + F1b attempted 2026-05-19 (on branch, real-device fail) + F1c SHIPPED 2026-05-19 PR #21** | Audit on `audit/v35-onboarding-glyph-clipping` branch off main `7eda10b` (PR #17 squash `f9023b1`), 437-line audit at `docs/audit/V35_onboarding_glyph_clipping_audit.md`. F1 (commit `4298053`, PR #20 closed) added `line-height: 1.5` to `.input` — failed real-device (~50% → ~10% visible). Audit's H1 was structurally wrong: `.input` targets uni-app's outer `<uni-input>` custom element, NOT the native `<input>`; framework hard-codes `uni-input { height: 1.4em }`. F1b (`4c2265e`, forward-add) reversed F1's line-height, added `height: 48px` override (matches login's `.form-input`) — cleared the framework height cap but EXPOSED a new bug: uni-app's `.uni-input-placeholder` is an absolutely-positioned overlay element (`top: auto` + flex-column = static-position fallback at wrapper content-box top) that overlays value text once the wrapper has visible height (at 23.8px the overlay was compressed into the value line and read as "clipping"; at 48px the overlap pattern became visually loud). F1c (forward-add on F1b, PR #21 second commit) hides the placeholder unconditionally via `:deep(.uni-input-placeholder) { display: none }` (3 LOC). Three-green cleared. Cross-refs: `v3_f1_glyph_clipping_shipped.md` (full F1→F1b→F1c history), `lesson_uni_input_wrapper_not_native.md`, `lesson_ios_safari_realdevice_gate.md`, `lesson_uni_app_placeholder_overlay.md` (NEW). F5 webfont anomaly still queued for separate sprint. |
