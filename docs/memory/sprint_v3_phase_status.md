...
| **v3.5 GC audit + F1 paused → O1 shipped** | Onboarding step 1 nickname input glyph clipping / onboarding flow removal | ✅ **AUDIT 2026-05-12 PR #17 + F1 attempted 2026-05-17 PR #20 closed + F1b attempted 2026-05-19 on branch + F1c attempted 2026-05-19 PR #21** → 🛑 **F1 SPRINT PAUSED 2026-05-20** (3 attempts all failed real-device) → ✅ **O1 SHIPPED 2026-05-20 PR #?** (onboarding flow removed entirely) | Audit on `audit/v35-onboarding-glyph-clipping` branch off main `7eda10b` (PR #17 squash `f9023b1`), 437-line audit at `docs/audit/V35_onboarding_glyph_clipping_audit.md`. F1 (`4298053`, line-height 1.5) failed real-device (~50% → ~10%). F1b (`4c2265e`, height 48px) cleared framework height cap but exposed placeholder overlay bug. F1c (`45fe92e`, `:deep(.uni-input-placeholder) { display: none }`) failed real-device despite Web Inspector + framework CSS verification predicting it would work — mechanism remains unknown. Per `lesson_blind_iteration_stop_after_3.md`, sprint paused after 3 failed attempts. O1 (`feat/o1-skip-onboarding-flow` off main `bc29524`) removes the entire onboarding wizard: deletes the `!u.onboarded_at` branch in App.vue's `enforceConsentGate`, swaps login signup-success redirect from `/pages/onboarding/index` to `/pages/index/index`. New users (tos_version='0') fall through to existing reconsent page via the gate's second branch. Migration 042 patches `handle_new_user` to read Google OAuth `full_name` for nickname default. Cross-refs: `o1_onboarding_removed.md`, `v3_f1_glyph_clipping_shipped.md`, `lesson_blind_iteration_stop_after_3.md`, `lesson_uni_input_wrapper_not_native.md`, `lesson_ios_safari_realdevice_gate.md`, `lesson_uni_app_placeholder_overlay.md`. F5 webfont anomaly now lower priority (onboarding page is orphan). |
