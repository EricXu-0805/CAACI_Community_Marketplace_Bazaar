...
| **v3.5 GC audit + F1b fix** | Onboarding step 1 nickname input glyph clipping | ✅ **AUDIT SHIPPED 2026-05-12 PR #17 + F1 attempted 2026-05-17 PR #20 (closed, real-device fail) + F1b SHIPPED 2026-05-19 PR #?** | Audit on `audit/v35-onboarding-glyph-clipping` branch off main `7eda10b` (PR #17 squash `f9023b1`), 437-line audit at `docs/audit/V35_onboarding_glyph_clipping_audit.md`. F1 (commit `4298053`, PR #20 closed) added `line-height: 1.5` to `.input` — failed real-device verification on iPhone Safari (~50% → ~10% visible). Audit's H1 was structurally wrong: `.input` targets uni-app's outer `<uni-input>` custom element, NOT the native `<input>`; framework hard-codes `uni-input { height: 1.4em }` (23.8px @ fs:17) which is the actual constraint. F1b (forward-add commit on the same branch) reverses F1's line-height, adds `height: 48px` override (matches login's `.form-input` prod-verified pattern), and changes `padding: 10px 0` → `padding: 0`. Three-green cleared. Cross-refs: `v3_f1_glyph_clipping_shipped.md` (full F1→F1b history), `lesson_uni_input_wrapper_not_native.md`, `lesson_ios_safari_realdevice_gate.md` (new HARD gate). F5 webfont anomaly still queued for separate sprint. |
