...
❌ **F1 attempted 2026-05-17** via `fix/f1-onboarding-glyph-clipping` branch (commit `4298053`, PR #20 closed). Added `line-height: 1.5` to `.input` per audit §6 F1 recommendation. **Failed real-device verification on iPhone Safari (Vercel preview): visible glyph dropped from ~50% to ~10% — F1 WORSENED the bug.** Audit's H1 was structurally wrong: `.input` SCSS targets uni-app's outer `<uni-input>` custom element (NOT the native `<input>`), and framework hard-codes `uni-input { height: 1.4em }` which is the actual cascade constraint. F1's unitless `line-height: 1.5` re-computed at inner real `<input>` via `line-height: inherit` against inner's own font-size = 25.5px, exceeding the 23.8px inner content-box by 1.7px → additional bottom clip via shadow-DOM `overflow: hidden`. PR #20 closed without merge.

❌ **F1b attempted 2026-05-19** via same `fix/f1-onboarding-glyph-clipping` branch (forward-add commit `4c2265e` on top of `4298053`, PR #21 first commit). Reversed F1's line-height, added `height: 48px` matching login's `.form-input` (cleared framework's `height: 1.4em` cap), changed `padding: 10px 0` → `padding: 0`. **Real-device verification on iPhone Safari (Vercel preview) revealed a NEW bug**: uni-app's `.uni-input-placeholder` is a separate `<span>` sibling of the inner real `<input>` inside `<div class="uni-input-wrapper">` — framework hardcodes `position: absolute; top: auto !important; color: gray; pointer-events: none`. Wrapper is `display: flex; flex-direction: column; justify-content: center`. For an absolutely-positioned child of a flex column container, `top: auto` resolves to the static-position fallback at the wrapper content-box TOP. With F1b's expanded 48px height, the placeholder cleanly separated above the centered value text → gray bar overlay on value chars. At pre-F1b 23.8px, the same overlay was compressed into the value line and read as the original "clipping" symptom (it was always overlay; the height made the pattern legible). uni-app's value-aware hide heuristic for the placeholder is unreliable on iOS Safari. F1b commit `4c2265e` kept on branch as forward-add history; F1c stacks on top.

❌ **F1c attempted 2026-05-19** via same `fix/f1-onboarding-glyph-clipping` branch (forward-add commit on top of `4c2265e`, PR #21 second commit). Added `:deep(.uni-input-placeholder) { display: none }` nested rule inside the existing `.input` block. 3 LOC + 6-line explanatory comment. Audit + framework CSS + Web Inspector data all predicted this would work. **Real-device verification on iPhone Safari Vercel preview: STILL FAILED — gray bar overlay STILL present on value text.** Mechanism remains unknown (open questions in `v3_f1_glyph_clipping_shipped.md` Sprint Outcome section). F1c commit `45fe92e` kept on branch as third audit-failed historical record.

🛑 **F1 sprint PAUSED 2026-05-20** per `lesson_blind_iteration_stop_after_3.md` — 3 fix attempts on the same bug all failed real-device verification is the threshold to STOP iterating and reassess scope. No F1d will be attempted.

✅ **SUPERSEDED by O1 SHIPPED 2026-05-20 PR #?** — entire onboarding flow removed instead of further iteration. `App.vue` gate's onboarding branch deleted; new users fall through to existing `/pages/reconsent/index` (legal consent preserved; nickname/campus/avatar all collected via other surfaces or auto-defaults). Closeout: `docs/memory/o1_onboarding_removed.md`. Also includes migration 042 patching `handle_new_user` to read Google OAuth `full_name` so new Google users get proper nicknames (Eric applies in Dashboard or via CLI). PR #20 (F1) + PR #21 (F1b/F1c) both closed without merge; their commits preserved on `fix/f1-onboarding-glyph-clipping` branch as learning chain.
