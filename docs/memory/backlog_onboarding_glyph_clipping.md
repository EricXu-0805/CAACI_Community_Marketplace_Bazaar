...
✅ **F1b FIX SHIPPED 2026-05-19** via same `fix/f1-onboarding-glyph-clipping` branch (forward-add commit on top of `4298053`, PR #?). Reverses F1's line-height, adds `height: 48px` to override framework's `height: 1.4em` cap (matches `login/index.vue`'s `.form-input` prod-verified pattern), changes `padding: 10px 0` → `padding: 0` (height explicit, no vertical pad needed). HARD gate: Vercel preview + real iPhone Safari verification required before squash-merge per new `lesson_ios_safari_realdevice_gate.md` rule. Closeout: `v3_f1_glyph_clipping_shipped.md` (rewritten with full F1→F1b history). 2 new lessons: `lesson_uni_input_wrapper_not_native.md` + `lesson_ios_safari_realdevice_gate.md`.
