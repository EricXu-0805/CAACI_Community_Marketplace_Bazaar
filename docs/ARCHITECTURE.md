...
> 最后更新：2026-07-19
> 仓库状态：未部署的 release candidate
> 生产边界：生产 Supabase migration ledger 与仓库仍有漂移；候选迁移、API
> 和前端文件存在于工作树不代表已经上线。
...
但 DCloud 锁定的 Vite dev-tool finding 与 `heic-to` LGPL 分发评审仍需按
[`NPM_DEPENDENCY_TRIAGE.md`](./NPM_DEPENDENCY_TRIAGE.md) 和
[`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md) 单独处理。
