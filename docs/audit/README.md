...
visual-refresh + onboarding sprints. **Every finding was triaged and the
actionable ones shipped** across QA rounds 1–6 (PRs #1–#140).
...
## What was covered (all resolved / shipped)
...
- **Security** — RLS coverage, `anon` EXECUTE grants (REVOKE FROM PUBLIC is not
  ...
  migrations + QA rounds 2–6; live posture re-verified with
  `has_function_privilege` before each prod change.
- **Accessibility / i18n / UX** — contrast, aria labels, bilingual (zh/en)
  coverage, empty + skeleton states. Shipped through the visual refresh and the
  QA rounds.
...
Audit reports are historical. For the live picture:
...
- `docs/SECURITY_SETUP.md` — current security configuration
